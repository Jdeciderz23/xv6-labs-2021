...
e1000_transmit(struct mbuf *m)
{
  ...
  // The descriptor cannot be reused until the device has completed its
  // previous transmission request.
...
e1000_recv(void)
{
  for(;;){
    ...
    // net_rx() may send an ARP reply, which calls e1000_transmit().
    // Do not hold the driver lock across that call.
    release(&e1000_lock);
    net_rx(m);
  }
}
